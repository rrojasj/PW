...
function buscarFactura() {
    ...
    var numFactBuscar = document.getElementById('buscarFactInput').value;
    var tablaCompras = localStorage.getItem('tablaCompras');
    var comprasArray = JSON.parse(tablaCompras);

    var facturaArray = recorrerArrayFacts(numFactBuscar, comprasArray);

    // var facturaEncontrada = comprasArray.find(factura => factura.numFact === numFactBuscar);

    if(facturaArray !== null) {
        var factura = facturaArray.numFact;
        var cliente = facturaArray.cliente;
        var fecha = facturaArray.fecha;
        var articulo = facturaArray.opcionArticulo;
        var cantidad = facturaArray.cant;
        var total = facturaArray.totalFactura;

        document.getElementById('bitacora-fact').innerHTML = "<b>Factura No.</b>" + factura;
        document.getElementById('bitacora-fecha').innerHTML = fecha;
        document.getElementById('bitacora-cliente').innerHTML = "<b>Cliente:</b> " + cliente;
        document.getElementById('bitacora-articulo').innerHTML = "<b>Artículo:</b> " + articulo;
        document.getElementById('bitacora-cantidad').innerHTML = "<b>Cantidad:</b> " + cantidad; 
        document.getElementById('bitacora-total').innerHTML = "<b>Total:</b> " + total; 

        activarBitacora();

    } 
    // else if (facturaArray == null){
    //     Swal.fire({
    //         icon: "error",
    //         title: "Oops...",
    //         html: `<b>No ha ingresado una número de factura</b><br>Trate nuevamente!`
    //     });
    // } 
    else {
        Swal.fire({
            icon: "error",
            title: "Oops...",
            html: `<b>La factura no ha sido encontrada!</b><br>Trate nuevamente!`
        }).then((result) => {
...
function recorrerArrayFacts(numFact, comprasArray){
    var facturaEncontrada = null;

    for(var i = 0; i < comprasArray.length; i++){
        // console.log(typeof(comprasArray[i].numFact));
        // console.log(typeof(numFact));
        if(comprasArray[i].numFact == numFact){
            
            facturaEncontrada = comprasArray[i];
            break;
        }
    }
...
/** 
 * @name activarBitacora
 * @description Remueve la clase d-none del div para que se muestre la sección agregando la clase "bitacora-fact"
 * */
function activarBitacora(){
    var elementosFact = document.getElementsByClassName('bitacora-fact');
...
function limpiarBusqueda(){
    document.getElementById('buscarFactInput').value = "";
}